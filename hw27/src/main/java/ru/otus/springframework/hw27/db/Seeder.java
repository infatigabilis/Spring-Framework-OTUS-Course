package ru.otus.springframework.hw27.db;
...
public class Seeder {
    ...
    private void seedComments() {
        commentRepository.save(new Comment("Aliquam eros leo, bibendum ac sem et", bookRepository.findById(1L).get()));
        commentRepository.save(new Comment("tempor cursus ante", bookRepository.findById(1L).get()));
        commentRepository.save(new Comment("Mauris erat quam, dapibus in massa vel, commodo pretium odio.", bookRepository.findById(1L).get()));
        commentRepository.save(new Comment("Praesent efficitur et arcu at scelerisque.", bookRepository.findById(2L).get()));
        commentRepository.save(new Comment("Nulla ac efficitur sapien", bookRepository.findById(3L).get()));
        commentRepository.save(new Comment("Mauris sit amet molestie est, et lacinia sem. In hac habitasse platea dictumst. Morbi ex metus", bookRepository.findById(5L).get()));
        commentRepository.save(new Comment("blandit vitae metus a, condimentum consectetur erat", bookRepository.findById(5L).get()));
    }
}
